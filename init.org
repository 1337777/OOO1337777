...
* Parametrism : parametric data, parametric functions
...
data which says /how/ successor (or how zero). This is precisely the =parametric /
polymorphic list / seq type= :
...
This parametric data type may indeed be instantiated to contain booleans =(seq bool)=
...
write _parametric / polymorphic functions_ which /only-touch the form/ of the
input. For example oneself may write one single function which compute the number form
(size) of any list (booleans list or numbers list or else) :
...
both arguments to some _composition_ / _map_ (parametric) function :
...
with =seq=. Primo, define the (non-parametric) _unit type / class_ which has some
